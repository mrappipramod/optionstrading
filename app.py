"""
NiftyEdge Pro — Streamlit Edition  v3.1
Dhan-powered Options Trading Dashboard
Fixes in v3.1:
  - CRITICAL: Intraday API now sends "YYYY-MM-DD HH:MM:SS" datetime strings
    (Dhan /charts/intraday requires full datetime, not plain date)
  - Historical API keeps plain "YYYY-MM-DD" date strings
  - Improved error messages distinguishing subscription vs date format issues
  - Auto-fallback suggestion when today has no data yet
Previous fixes (v3.0):
...
def verify_credentials(client_id, access_token):
    ...
    try:
        ...
        if r.status_code in (401, 403):
            try:
                msg = r.json().get("errorMessage") or r.json().get("message") or r.text[:200]
            except:
                msg = r.text[:200]
        ...
        return False, "Timeout — check internet"
    except requests.exceptions.ConnectionError:
        return False, "Cannot reach Dhan API"
    except Exception as e:
        return False, str(e)
...
def resolve_date_range(date_mode, custom_from=None, custom_to=None, interval="15"):
    ...
    Dhan's intraday endpoint rejects plain date strings and silently returns
    an empty close array, which appears as "No candle data" in the UI.
    """
    ...
    if is_daily:
        # Historical endpoint expects plain YYYY-MM-DD
        return (str(fd), str(td))
    else:
        # ── CRITICAL FIX ──────────────────────────────────────────────────
        # Intraday endpoint REQUIRES full datetime strings.
        # Plain "YYYY-MM-DD" causes silent empty response from Dhan API.
        # Market session: 09:15 IST open → 15:30 IST close.
        # ──────────────────────────────────────────────────────────────────
        from_str = f"{fd} 09:15:00"
        to_str   = f"{td} 15:30:00"
...
def fetch_candles(symbol, interval="15", date_mode="Today",
    ...
    if is_daily:
        ...
        body = {
            ...
            "interval":        interval,
            "oi":              False,
            "fromDate":        from_str,   # now "YYYY-MM-DD HH:MM:SS"
            "toDate":          to_str,     # now "YYYY-MM-DD HH:MM:SS"
        }
        raw = dhan_post("/charts/intraday", body)

    # Normalise: handle optional "data" wrapper
    if isinstance(raw, dict) and "data" in raw and isinstance(raw["data"], dict):
        raw = raw["data"]

    # Validate candle arrays
    if isinstance(raw, dict) and not raw.get("error"):
        ...
        if isinstance(close_arr, list) and len(close_arr) == 0:
            ...
            hint = ""
            if date_mode == "Today" and not is_daily:
                hint = " If it's before 09:15 IST or after 15:30 IST, no intraday data exists yet — try 'Last Trading Day'."
            raw = {
                "error": (
                    f"No candles returned for {symbol} ({date_mode}). "
                    f"Sent: fromDate={from_str}, toDate={to_str}."
                    f"{hint} "
                    "Other causes: Data API subscription not active (₹499/mo at api.dhan.co), "
                    "or Security ID mismatch."
                )
            }
    return raw
...
def search_dhan_instrument(query: str, segment: str = "NSE_EQ"):
    """Search Dhan's live instrument list. Cached 1 hour."""
    try:
...
def calc_pnl(entry_price, exit_price, qty, direction, trade_type,
    ...
    if "FNO" in trade_type:
        ...
        stt = total_turnover * 0.00025
    else:
        stt = turnover_exit * 0.001

    if "FNO" in trade_type:
        exc = total_turnover * 0.000053
    else:
        exc = total_turnover * 0.0000322

    sebi  = total_turnover * 0.000001
    gst   = (brok + exc + sebi) * 0.18

    if "FNO" in trade_type:
        stamp = turnover_entry * 0.00002
    elif "INTRADAY" in trade_type:
...
def build_chart(candles, candle_signals, indicators, show_ema, show_vwap, show_bb):
    ...
    vcols = [
        "rgba(0,212,170,0.5)" if r["c"] >= r["o"] else "rgba(248,113,113,0.5)"
        for _, r in df.iterrows()
    ]
    fig.add_trace(go.Bar(x=df["dt"], y=df["v"], marker_color=vcols,
                         name="Vol", showlegend=False), row=2, col=1)

    if "macd_hist" in df.columns:
        mh    = df["macd_hist"]
        mcols = ["rgba(0,212,170,0.6)" if v >= 0 else "rgba(248,113,113,0.6)" for v in mh]
        fig.add_trace(go.Bar(x=df["dt"], y=mh, marker_color=mcols,
    ...
    fig.update_xaxes(
        rangeselector=dict(
            ...
            bgcolor="#ffffff", activecolor="#f0f4f8",
...
def show_option_chain_tab(sym, ltp):
        ...
        if is_likely_fno:
            ...
            <div style='background:#fef9ec;border:1.5px solid #f59e0b44;border-radius:12px;
              ...
              <div style='color:#64748b;font-size:.83rem;line-height:1.8;'>
                <b style='color:#1e293b;'>{sym_name}</b> ({seg_disp}) does not appear to be
                in the NSE F&O segment, or no active expiries were found.<br>
                ...
                <b style='color:#1e293b;'>🎯 Strategies</b> tabs for full technical analysis.
              </div>
    ...
    max_oi  = max(
        max((r["ce_oi"] for r in rows), default=1),
    ...
    view = st.radio("View", ["Full Chain","CE Focus","PE Focus"],
                    horizontal=True, label_visibility="collapsed")

    if view == "Full Chain":
        hdr = """<table class='oc-table'><tr>
        ...
        hdr = """<table class='oc-table'><tr>
          <th>CE OI</th><th>Δ OI</th><th>IV</th><th>Delta</th><th>Theta</th>
          <th>LTP</th><th>Bid</th><th>Ask</th><th>STRIKE</th></tr>"""
    else:
        hdr = """<table class='oc-table'><tr>
          <th>STRIKE</th><th>Bid</th><th>Ask</th><th>LTP</th><th>Delta</th>
          <th>Theta</th><th>IV</th><th>PE OI</th><th>Δ OI</th></tr>"""
    ...
    for r in atm_rows:
        ...
        if view == "Full Chain":
            rows_html += f"""<tr class='{atm_cls}'>
              <td class='oc-call'>{fmt_oi(r['ce_oi'])}
              ...
              <td class='oc-put'>{fmt_oi(r['pe_oi'])}
            ...
            if st.button(f"🟢 BUY {chosen_strike} {opt_type.split()[0]}",
                ...
                if not sec_id_opt:
                    ...
                    if res.get("orderId"):
                        ...
                        add_alert("BUY", f"BUY {sym} {chosen_strike} {opt_type.split()[0]} "
                                         f"×{lots*lot} @ {'MKT' if limit_price==0 else limit_price}")
                    else:
            ...
            if st.button(f"🔴 SELL {chosen_strike} {opt_type.split()[0]}",
                ...
                if not sec_id_opt:
                    ...
                    if res.get("orderId"):
                        ...
                        add_alert("SELL", f"SELL {sym} {chosen_strike} {opt_type.split()[0]} "
                                          f"×{lots*lot} @ {'MKT' if limit_price==0 else limit_price}")
                    else:
                        st.error(res.get("errorMessage") or res.get("error") or str(res))
        with b3:
            st.caption("⚠️ Orders execute on NSE_FNO segment. Verify Security ID before placing.")


# ─── PORTFOLIO TAB ────────────────────────────────────────────────────────────
def show_portfolio_tab():
    for key in ("portfolio_funds","portfolio_pos","portfolio_hld","portfolio_orders"):
    ...
    if funds and not funds.get("error"):
        avail = float(
            funds.get("availabelBalance") or funds.get("availableBalance") or
    ...
    with t1:
        ...
        if pos is None:
            ...
            keep   = [c for c in [
                "tradingSymbol","exchangeSegment","positionType","productType",
    ...
    with t2:
        ...
        if hld is None:
            ...
            keep_h = [c for c in [
                "tradingSymbol","exchange","totalQty","availableQty",
                "avgCostPrice","t1Qty","isin",
            ] if c in df_hld.columns]
            df_h2  = df_hld[keep_h].copy()
            df_h2.columns = [
                c.replace("tradingSymbol","Symbol").replace("totalQty","Total Qty")
                 .replace("availableQty","Avail Qty").replace("avgCostPrice","Avg Cost")
                 .replace("t1Qty","T1 Qty")
                for c in keep_h
            ]
    ...
    with t3:
        ...
        if orders is None:
            ...
            keep_o = [c for c in [
                "orderId","tradingSymbol","transactionType","orderStatus",
                "orderType","quantity","price","productType","createTime",
            ] if c in df_ord.columns]
            df_o2  = df_ord[keep_o].copy()
            df_o2.columns = [
                c.replace("tradingSymbol","Symbol").replace("transactionType","Side")
            ...
            if "Status" in df_o2.columns:
                ...
                status_colors = {
                    "TRADED":"#059669","CANCELLED":"#dc2626","REJECTED":"#dc2626",
    ...
    with t4:
        ...
        if trade_log:
            ...
            with jc1:
                if st.button("🗑️ Clear All Trades", type="secondary",
                             use_container_width=True, key="jclear"):
        ...
        with st.expander("➕ Log a Trade Manually", expanded=not bool(trade_log)):
            ...
            if st.button("📥 Calculate & Log Trade", use_container_width=True,
                ...
                st.success(
                    f"✅ Logged! Net P&L: ₹{pnl_data['net_pnl']:+,.2f} "
...
def show_login():
    ...
    with col:
        ...
        <div style='background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;
          ...
          <div style='font-size:2rem;font-weight:800;color:#00d4aa;text-align:center;margin-bottom:.3rem;'>
            ⚡ NiftyEdge Pro
          </div>
...
- Option Chain / Data APIs need **Data API subscription** (₹499/month)
        ...
        st.markdown(
            "<div style='text-align:center;margin-top:1rem;font-size:.75rem;color:#64748b;'>"
            "🔒 Stored in browser session only — never sent to third parties<br>"
            "<a href='https://api.dhan.co' target='_blank' style='color:#00d4aa'>api.dhan.co</a>"
            "</div>",
...
def show_dashboard():
    ...
    with st.sidebar:
        st.markdown(
            f"<div style='font-size:1.15rem;font-weight:700;color:#00d4aa;'>⚡ NiftyEdge Pro</div>"
            f"<div style='font-size:.7rem;color:#64748b;margin-bottom:.6rem;'>"
            f"{st.session_state.client_id[:6]}•••• · {len(SECURITY_IDS)} instruments</div>",
            unsafe_allow_html=True,
        )

        st.markdown(
            "<div style='font-size:.75rem;color:#64748b;font-weight:600;margin-bottom:4px;'>"
            "🔍 Search Any Stock / Index</div>",
            unsafe_allow_html=True,
        )
        ...
        if sq:
            ...
            if opts:
                ...
                with bc2:
                    ...
                    st.markdown(
                        f"<div style='font-size:.65rem;padding-top:7px;"
                        f"color:{'#00d4aa' if oc_ok else '#f59e0b'};'>"
                        f"{'✅ F&O' if oc_ok else '⚠️ No OC'}</div>",
                        unsafe_allow_html=True,
                    )
            else:
            ...
            with st.expander("🔧 Load any scrip by Security ID"):
                ...
                live_q   = st.text_input("Search Dhan instruments",
                                         placeholder="e.g. ETERNAL, PAYTM", key="cs_live")
                ...
                if live_q and len(live_q) >= 2:
                    ...
                    if live_res:
                        for lr in live_res[:8]:
                            ...
                            with lc1:
                                st.markdown(
                                    f"<div style='font-size:.72rem;color:#1e293b;'>"
                                    f"{lr['symbol']} <span style='color:#64748b;'>ID:{lr['id']}</span><br>"
                                    f"<span style='color:#64748b;font-size:.65rem;'>{lr['name']}</span></div>",
                                    unsafe_allow_html=True,
                                )
                            with lc2:
                                if st.button("Load", key=f"live_load_{lr['id']}", use_container_width=True):
                                    ...
                                    st.success(f"Loaded {sym_key}")
                                    st.rerun()
                    else:
                        st.caption("Not found. Try the manual entry below.")

                st.markdown(
                    "<div style='font-size:.65rem;color:#64748b;margin-top:6px;'>Manual entry:</div>",
                    unsafe_allow_html=True,
                )
                ...
                if st.button("➕ Add & Load", use_container_width=True, key="cs_add"):
                    if c_sym and c_id:
                        ...
                        if sym_key not in SECTOR_GROUPS["Custom"]:
                            SECTOR_GROUPS["Custom"].append(sym_key)
                        st.session_state.symbol = sym_key
                        st.success(f"Added {sym_key}")
                        st.rerun()
                    else:
                        st.error("Both fields required")
        else:
            st.markdown(
                "<div style='font-size:.72rem;color:#64748b;margin-bottom:3px;'>📌 Major Instruments</div>",
                unsafe_allow_html=True,
            )
            MAJORS = {
            ...
            for grp, gsyms in MAJORS.items():
                st.markdown(
                    f"<div style='font-size:.63rem;color:#64748b;margin-top:4px;'>{grp}</div>",
                    unsafe_allow_html=True,
                )
                gc = st.columns(3)
                for i, gs in enumerate(gsyms):
                    with gc[i % 3]:
                        ...
                        if st.button(gs, key=f"maj_{gs}", use_container_width=True,
                            ...
                            st.session_state.symbol = gs
                            st.rerun()

            st.markdown(
                "<div style='font-size:.72rem;color:#64748b;margin:6px 0 3px;'>🗂 Browse by Sector</div>",
                unsafe_allow_html=True,
            )
        ...
        # ── TIMEFRAME ────────────────────────────────────────────────────────
        st.markdown(
            "<div style='font-size:.75rem;color:#64748b;font-weight:600;margin-bottom:3px;'>⏱ Timeframe</div>",
            unsafe_allow_html=True,
        )
        tf_map = {
        ...
        # ── DATE RANGE ───────────────────────────────────────────────────────
        st.markdown(
            "<div style='font-size:.75rem;color:#64748b;font-weight:600;margin:8px 0 3px;'>📅 Date Range</div>",
            unsafe_allow_html=True,
        )
        ...
        if date_mode == "Custom":
            ...
            with col_f:
                default_from = custom_from if custom_from else (
                    ltday if not is_daily else today - timedelta(days=90)
                )
                new_from = st.date_input("From", value=default_from, max_value=today)
            with col_t:
                default_to = custom_to if custom_to else today
                new_to     = st.date_input("To",   value=default_to,   max_value=today)
            ...
            td_disp = td.split(" ")[0]
            st.markdown(
                f"<div style='font-size:.68rem;color:#64748b;padding:3px 0;'>"
                f"{fd_disp} → {td_disp}</div>",
                unsafe_allow_html=True,
            )

        st.markdown(
            "<div style='font-size:.75rem;color:#64748b;font-weight:600;margin:8px 0 3px;'>🎨 Chart Overlays</div>",
            unsafe_allow_html=True,
        )
        ...
        if st.button("💰 Funds", use_container_width=True):
            ...
            if isinstance(f, dict) and not f.get("error"):
                av = (
                    f.get("availabelBalance") or f.get("availableBalance") or
                    (f.get("data") or {}).get("availabelBalance", 0)
                )
    ...
    <div style='display:flex;justify-content:space-between;align-items:center;
      ...
      <span style='font-size:.85rem;color:#1e293b;font-weight:600;'>{sym}
        <span style='color:#64748b;font-weight:400;font-size:.73rem;'>
          {sym_inf.get("name","")} · {sym_inf.get("sector","")}
        </span>
      </span>
      <span style='font-size:.78rem;'>
        <span style='color:{mkt_col};font-weight:700;'>{mkt_emoji} {mkt_status}</span>
    ...
    ltp, chg, pct = _extract_ltp(quote, SECURITY_IDS[sym]["segment"], SECURITY_IDS[sym]["id"])
    ...
    cache_ttl  = 90 if live_mode else 3600
        ...
        if has_data:
            try:
                ...
                if sig.get("type") in ("BUY","SELL"):
                    ...
                    opt_str   = (
                        "→ BUY CE 📈" if stype_sig == "BUY" else "→ BUY PE 📉"
                    ) if has_option_chain(sym) else (
                        "→ BUY stock" if stype_sig == "BUY" else "→ SELL/SHORT"
                    )
                    add_alert(stype_sig, f"{sym} {pat_str} ({tf_lbl}) {opt_str}")
            except Exception as e:
            ...
            if err:
                ...
                st.error(
                    ...
                    "Data API subscription required (₹499/mo at api.dhan.co), "
                    "or Security ID mismatch. Try 'Last 5 Days' or 'Last Trading Day'."
                )
    ...
    with tab_chart:
        ...
        with ccol:
            if result:
                ...
                with tb2:
                    ...
                    fd_show    = fd_d.split(" ")[0]
                    td_show    = td_d.split(" ")[0]
                    candle_count = len(result.get("candles", []))
                    st.markdown(
                        f"<span style='font-size:.72rem;color:#64748b;line-height:2.2;'>"
                ...
                fig = build_chart(
                    result.get("candles"), result.get("candle_signals"),
                ...
                st.caption(
                    f"Updated {datetime.fromtimestamp(ts).strftime('%H:%M:%S')} IST · "
                    f"{tf_lbl} · {date_mode} · {refresh_lbl}"
                )
            else:
        ...
        with rcol:
            ...
            if result:
                ...
                rsi_v = ind.get("rsi", 50)
                close = (
                    result.get("candles", [{}])[-1].get("c", ltp)
                    if result.get("candles") else ltp
                )
                atr   = ind.get("atr", close * 0.005)

                is_fno    = has_option_chain(sym)
                strike_gap = (
                    50  if sym in ("NIFTY","BANKNIFTY","SENSEX","BANKEX") else
                    100 if sym in ("FINNIFTY","MIDCPNIFTY") else
                    50  if close > 5000 else
                    20  if close > 1000 else 10
                )
                ...
                if is_fno and stype != "NEUTRAL":
                    fno_action_html = f"""
                      <div style='margin-top:8px;padding:8px 10px;background:#f1f5f9;
                        ...
                        <div style='font-size:.68rem;color:#64748b;'>Aggressive: {otm_lbl}</div>
                ...
                def vb(c, t, f):
                    return (
                        f'<span style="color:#00d4aa">{t}</span>' if c
                ...
                st.markdown(f"""<div style='margin:8px 0 2px'>
                  <div style='height:5px;border-radius:3px;background:#f1f5f9;overflow:hidden'>
                    <div style='height:100%;width:{rp}%;background:{bc2};border-radius:3px'></div>
                  </div>
            ...
            for a in alerts[:8]:
                ...
                st.markdown(
                    ...
                    f"<div class='adot' style='background:{c}'></div>"
                    f"<div><span style='color:{c};font-weight:700'>{a['type']}</span>"
                    f"<span style='color:#64748b'> {a['text']}</span>"
                    f"<div style='font-size:.65rem;color:#64748b'>{a['time']}</div>"
                    f"</div></div>",
                    unsafe_allow_html=True,
                )
...
def _log_closed_trade(active_trade, sym, pnl_pct, exit_reason):
    ...
    exit_price = (
        round(entry * (1 + pnl_pct / 100), 2) if direction == "LONG"
        else round(entry * (1 - pnl_pct / 100), 2)
    )
...
def show_strategy_tab(result, sym, tf_lbl):
    ...
    cd = con["consensus"]
    cc = (
        "#00d4aa" if "BUY"  in cd else
        "#f87171" if "SELL" in cd else
        "#f59e0b" if cd in ("STOP_LOSS","EXIT") else
        "#4a6fa5"
    )
    em = (
        "🟢" if "BUY"  in cd else
        "🔴" if "SELL" in cd else
        "⚠️" if cd in ("STOP_LOSS","EXIT") else
        "⏸️"
    )

    st.markdown(f"""
    <div style='background:{cc}15;border:1.5px solid {cc}44;border-radius:12px;
         padding:14px 18px;margin-bottom:12px;
         display:flex;justify-content:space-between;align-items:center;'>
      <div>
        <div style='font-size:.7rem;color:#64748b;margin-bottom:2px;'>
          CONSENSUS — {sym} ({tf_lbl})
        </div>
            ...
            if act in ("ENTER_LONG","ENTER_SHORT"):
                ...
                if setup.in_trade:
                    ...
                    if mile:
                        ...
                        st.markdown(
                            f"<div style='background:{mc3}22;border:1px solid {mc3}55;"
                            f"border-radius:6px;padding:8px;color:{mc3};font-weight:700;'>"
                            f"🎯 {mile.replace('_',' ')}</div>",
                            unsafe_allow_html=True,
                        )
                ...
                if setup.in_trade and st.button("🚪 Close", key=f"cls_{name}"):
                    ...
                    add_alert("INFO", f"{name}: Manual exit")
                    st.success("Closed")
                    st.rerun()
                ...
                st.markdown(
                    f"<div style='background:#eff6ff;border:1px solid #60a5fa44;"
                    f"border-radius:8px;padding:10px;'>"
                    f"<span style='color:#60a5fa;font-weight:700;font-size:1.1rem;'>EXIT</span>"
                    f"<span style='color:{pc};margin-left:10px;'>{pnl:+.2f}%</span></div>",
                    unsafe_allow_html=True,
                )
                ...
                if st.button("✅ Confirm Exit", key=f"ext_{name}"):
                    ...
                    add_alert("INFO", f"{name}: EXIT {pnl:+.2f}%")
                    st.rerun()
                ...
                st.markdown(
                    ...
                    f"<div style='font-size:1.2rem;font-weight:800;color:#ef4444;'>🚨 STOP LOSS HIT</div>"
                    f"<div style='color:#f87171;font-size:.85rem;'>Loss: {pnl:+.2f}%</div></div>",
                    unsafe_allow_html=True,
                )
                ...
                if st.button("🚨 Confirm SL Exit", key=f"sl_{name}", type="primary"):
                    ...
                    add_alert("SELL", f"{name}: SL {pnl:+.2f}%")
                    st.rerun()
            else:
    ...
    if active:
        ...
        for strat, trade in active.items():
            ...
            pnl   = (
                ((close - entry) / entry * 100) if d == "LONG" and entry
                else ((entry - close) / entry * 100) if entry else 0
            )
            pc = "#00d4aa" if pnl >= 0 else "#f87171"
            st.markdown(
                ...
                f"<div><b style='color:#1e293b'>{strat}</b>"
                f"<span style='color:#64748b;font-size:.73rem;margin-left:8px;'>"
                f"{d} · @{entry:.0f} · SL {trade.get('stop_loss',0):.0f}</span></div>"
                f"<div style='font-weight:700;color:{pc}'>{pnl:+.2f}%</div></div>",
                unsafe_allow_html=True,
            )
    ...
    for col, (pname, pdata) in zip([p1, p2, p3, p4], PROFILES.items()):
        with col:
            ...
            if st.button("Apply", key=f"preset_{pname}", use_container_width=True):
                ...
                st.success(f"{pname} profile applied!")
                st.rerun()

    st.divider()

    st.markdown("#### 🔧 Risk Management")
    st.caption("Professional traders never risk more than 1–2% per trade.")

    with st.expander("📐 Stop Loss & Position Sizing", expanded=True):
        rc1, rc2 = st.columns([3, 2])
        with rc1:
            sl_atr = st.slider("Stop Loss — ATR Multiplier", 0.5, 3.0,
                               float(cfg.get("sl_atr_mult", 1.5)), 0.1)
            t1_rr  = st.slider("Target 1 — Risk:Reward",   0.5, 3.0,
                               float(cfg.get("t1_rr", 1.5)),     0.25)
            t2_rr  = st.slider("Target 2 — Risk:Reward",   1.5, 6.0,
                               float(cfg.get("t2_rr", 2.5)),     0.25)
            t3_rr  = st.slider("Target 3 — Runner / Trail",2.0, 10.0,
                               float(cfg.get("t3_rr", 4.0)),     0.5)
        with rc2:
            risk   = 100
            ...
            <div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px;'>
              ...
              <div style='font-size:.68rem;color:#64748b;border-top:1px solid #dde3ec;padding-top:8px;'>
                💡 Never enter unless T2 ≥ 2× risk.<br>
                Trail SL to entry after T1 hits.
              </div>
            </div>""", unsafe_allow_html=True)
        cfg.update(sl_atr_mult=sl_atr, t1_rr=t1_rr, t2_rr=t2_rr, t3_rr=t3_rr)

    st.markdown("#### 🎯 Entry Filters")
    st.caption("Stricter filters = fewer but higher-quality trades.")

    with st.expander("📊 EMA Trend Strategy", expanded=False):
        col1, col2 = st.columns([2, 1])
        with col1:
            ms = st.slider("Minimum Signal Score", 20, 70, int(cfg.get("min_score", 30)), 5)
        with col2:
        ...
        cfg["min_score"] = ms
        st.info("💡 Score ≥ 40 for day trading, ≥ 50 for swing. Lower = more whipsaws.")

    with st.expander("📉 RSI Mean Reversion Strategy", expanded=False):
        rc1, rc2 = st.columns(2)
        with rc1:
            rsi_os = st.slider("RSI Oversold",  20, 40, int(cfg.get("rsi_os", 30)), 1)
        with rc2:
            rsi_ob = st.slider("RSI Overbought",60, 80, int(cfg.get("rsi_ob", 70)), 1)
        cfg.update(rsi_os=rsi_os, rsi_ob=rsi_ob)
        st.info("💡 Classic 30/70. Use 35/65 for faster signals on F&O names.")

    with st.expander("🎯 Multi-Confluence Strategy", expanded=False):
        mc = st.slider("Minimum Confluences Required", 2, 6, int(cfg.get("min_confluences", 4)), 1)
        ...
        for i, (col_c, lbl) in enumerate(zip(cols_c, labels)):
            ...
            <div style='text-align:center;padding:6px 4px;border-radius:6px;
                 background:{"#ecfdf5" if active_chip else "#f8fafc"};
                 border:1px solid {"#059669" if active_chip else "#dde3ec"};'>
              <div style='font-size:.6rem;color:{"#059669" if active_chip else "#94a3b8"};
                   font-weight:{"700" if active_chip else "400"};'>{lbl}</div>
              <div style='font-size:.8rem;'>{"✅" if active_chip else "○"}</div>
            </div>""", unsafe_allow_html=True)
        ...
        st.info("💡 Professional standard: 4/6 confluences for best balance.")
...
Never risk more than 1% of your capital on a single trade. On ₹5L capital, that's ₹5,000 max risk.

**2. Minimum 1:2 Risk:Reward**
Never take a trade where you can't make at least 2× what you're risking. You can be right only 40% of the time and still be profitable.
...
On 15-min chart, only take trades in the direction of the 1-hour trend. Check EMA stack on a higher timeframe first.
...
When MACD and RSI conflict, or VIX is above 20, reduce position size by 50%. Uncertainty = smaller size.
    ...
    strategy_guide = {
        "EMA Trend Follow": {
            ...
            "pro_tip": "Most reliable on high-volume stocks and BankNifty. Avoid in first 15 min.",
        },
        "RSI Mean Reversion": {
            "when":    "Range-bound, low-volatility sessions — Nifty stuck in a range",
            "avoid":   "Strong trends — RSI can stay at extremes for hours",
            "entry":   "RSI < 30 + price at BB lower + Hammer / Bullish Engulfing",
            "sl":      "Below the low of the reversal candle",
            "target":  "BB mid (T1), BB upper (T2)",
        ...
        "MACD Momentum": {
            ...
            "entry":   "MACD line × signal cross + histogram expansion + EMA alignment",
            "sl":      "ATR × 2.0 (wider — momentum trades need room to breathe)",
            "target":  "Let it run with trailing SL",
            "best_tf": "15 or 25 min chart",
            "pro_tip": "Valid momentum breakouts always have high volume confirmation.",
        },
        "VWAP Reversal": {
            "when":    "Intraday scalps — price testing VWAP level",
            "avoid":   "First 15 min (VWAP unreliable) and last 30 min (expiry noise)",
            "entry":   "Price reclaims VWAP from below + increasing volume + bullish candle",
            "sl":      "Just below VWAP (1 ATR × 0.5)",
            "target":  "T1 near prior swing high",
            "best_tf": "5 min chart with 15 min trend filter",
            "pro_tip": "VWAP is most powerful when price makes a big move away then comes back.",
        },
        "Multi-Confluence": {
            "when":    "Any market — only enter when 4+ signals align",
            "avoid":   "When in a hurry — requires patience. Can go all day without a setup.",
            "entry":   "4–5 of: EMA Stack + MACD + RSI Zone + VWAP + BB + Candle Pattern",
            "sl":      "ATR × 1.8 — highest conviction = can afford wider SL",
            "target":  "T3 is realistic — high-confidence trades often run far",
            "best_tf": "15 or 25 min for best results",
            "pro_tip": "Top traders miss 80% of moves but capture the cleanest 20% with precision.",
        },
    }
